...
def _handle_session_sse_stream(handler, parsed):
    ...
    end-of-turn). Used to deliver ``process_complete`` events that fire while
...
def start_session_turn(
    ...
    *,
    ...
    # Server-initiated wakeup (Option Z): the session record ALREADY has a
    # persisted model/provider (set by _prepare_chat_start_session_for_stream
    # on a prior turn). Resolve it WITHOUT a live provider-catalog rebuild —
    # the drain thread fires while idle so the catalog is cold, and the live
    # rebuild does a Copilot token-exchange HTTPS call that hangs the wakeup
    # chat/start on this WSL/corp network (proven thread-stack, impl report
    # §1). prefer_cached_catalog serves the warm/disk cache or a network-free
    # minimal catalog; the persisted session model still wins, so the wakeup
    # turn always gets a valid model (persisted → static/default), never a
...
def _handle_process_complete_ack(handler, body):
    """Acknowledge a process_complete SSE event (diagnostic only).
    ...
    (analytics, telemetry) has a stable hook. ``PENDING_PROCESS_COMPLETIONS``
